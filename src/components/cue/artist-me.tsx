      ...
      <section className="mt-8 px-5">
        ...
        <form onSubmit={onUpload} className="mt-3 space-y-2">
          ...
          <div className="flex gap-2">
            <button type="button" onClick={() => fileRef.current?.click()} className="flex h-11 flex-1 items-center justify-center truncate rounded-md bg-elevated px-3 text-sm">{fileName ?? "Choose audio file"}</button>
            <Button type="submit" size="md" disabled={busy}>{busy ? "Sending…" : "Upload"}</Button>
          </div>
        ...
        {pending.length + live.length === 0 ? <p className="mt-4 text-sm text-subtle">Nothing in the queue yet.</p> : (
          <ul className="mt-4 divide-y divide-line border-y border-line">
            {[...pending, ...live].map((song) => <SongLinksRow key={song.id} song={song} onSave={(extra) => updateSongLinks(song.id, extra)} />)}
          </ul>
        )}
      </section>
