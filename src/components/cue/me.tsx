    ...
    >
      ...
      <div className="mt-6 space-y-4">
        ...
        <Field label="I am a">
          <SelectInput
            value={kind}
            onChange={(e) => setKind(e.target.value as "artist" | "explorer" | "business")}
          >
        ...
        {kind === "artist" ? (
          <Field label="Role">
            <TextInput
              ...
              placeholder="Vocalist, drummer, producer…"
              required
            />
          </Field>
        ) : null}
        ...
        {kind === "artist" ? (
          <div>
            ...
            <button
              ...
              className="mt-2 flex h-11 w-full items-center justify-center truncate rounded-md bg-elevated px-3 text-sm"
            >
              {fileName ?? "Choose audio file"}
            </button>
            <p className="mt-2 text-xs italic text-subtle">
              One track is required before Inner Soul Records can verify you.
            </p>
          </div>
        ) : null}
      </div>
    ...
    <div className="cue-enter pb-12">
      ...
      <div className="flex items-end gap-4 px-5">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs italic text-subtle">Shown on your artist page and the roster.</p>
        </div>
      </div>
      ...
      <form onSubmit={onSave} className="mt-6 space-y-3 px-5">
        ...
        <p className="text-xs italic text-subtle">
          Links only show on your public profile if they are filled in.
        </p>
      ...
      <section className="mt-8 px-5">
        ...
        <p className="mt-2 text-sm text-muted">Uploads wait for approval before they go live. Audio plus a cover if you have one.</p>
        <form onSubmit={onUpload} className="mt-3 space-y-2">
          <div className="flex items-start gap-3">
            <PhotoPick
              src={trackCover ?? "/media/covers/vinyl.jpg"}
              label="Choose cover art"
              className="size-16 shrink-0"
              onChange={setTrackCover}
            />
            <p className="pt-1 text-xs leading-5 text-subtle">
              Cover art sits on your artist page, Discover, and the player — same place as the catalogue sleeves.
            </p>
          </div>
          <TextInput
            value={title}
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => fileRef.current?.click()}
              className="flex h-11 flex-1 items-center justify-center truncate rounded-md bg-elevated px-3 text-sm"
            >
              {fileName ?? "Choose audio file"}
            </button>
          ...
          <ul className="mt-4 divide-y divide-line border-y border-line">
            {[...pending, ...live].map((song) => (
              <SongLinksRow
                key={song.id}
                song={song}
                onSave={(extra) => updateSongLinks(song.id, extra)}
              />
            ))}
          </ul>
        )}
  ...
  return (
    <li className="py-3">
      <div className="flex items-center gap-3">
        <PhotoPick
          src={song.cover}
          label={`Change cover art for ${song.title}`}
          className="size-12 shrink-0"
          onChange={(cover) => onSave({ spotify: sp, youtube: yt, cover })}
        />
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-medium">{song.title}</p>
          <p className="text-xs text-muted">{song.status === "approved" ? "Live" : "Awaiting approval"}</p>
        </div>
        <span className="rounded-md bg-elevated px-2 py-1 text-xs uppercase tracking-wide text-muted">
          {song.status}
        </span>
      </div>
      <div className="mt-2 space-y-2">
        <TextInput
          ...
          placeholder="Spotify URL for this song"
          aria-label="Spotify URL for this song"
        />
        <TextInput
          type="url"
          value={yt}
          onChange={(e) => setYt(e.target.value)}
          placeholder="YouTube URL for this song"
          aria-label="YouTube URL for this song"
        />
        {dirty ? (
          <Button
            type="button"
            ...
            size="sm"
            className="w-full"
            onClick={() => onSave({ spotify: sp, youtube: yt })}
          >
            Save song links
          </Button>
        ) : null}
  ...
  return (
    <div className="cue-enter pb-12">
      <ScreenHead
        kicker="You"
        title="Profile"
        note={acc.kind === "business" ? KIND_LABEL.business : KIND_LABEL.explorer}
      />
      <div className="flex items-end gap-4 px-5">
        <PhotoPick
          src={acc.photo}
