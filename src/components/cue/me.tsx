...
import { kindLabel, locationLabel, storeErr, useLocale, useT } from "@/lib/i18n";
...
function ResetForm() {
  ...
  return (
    <form className="cue-enter px-5 pb-10 pt-5" onSubmit={(e) => { e.preventDefault(); if (password !== confirm) { setError(t("passwordsMismatch")); return; } const err = resetPassword(email, password); if (err) { setError(err); return; } setError(null); setDone(true); }}>
      <p className="cue-kicker text-xs text-muted">{t("account")}</p>
      ...
      {done ? (
        ...
        <>
          ...
          {error ? <p className="mt-3 text-sm text-accent">{storeErr(locale, error) ?? error}</p> : null}
...
function RegisterForm() {
  ...
  return (
    <form className="cue-enter px-5 pb-12 pt-5" onSubmit={(e) => { e.preventDefault(); if (kind === "artist") { if (!fileName) { setError(t("errUploadMp3")); return; } setTermsOpen(true); return; } submitRegister(); }}>
      <p className="cue-kicker text-xs text-muted">{t("account")}</p>
      ...
      <div className="mt-6 space-y-4">
        ...
        <Field label={t("iAmA")}><SelectInput value={kind} onChange={(e) => setKind(e.target.value as "artist" | "explorer" | "business")}><option value="artist">{t("kindArtist")}</option><option value="explorer">{t("kindExplorer")}</option><option value="business">{t("kindBusiness")}</option></SelectInput></Field>
        ...
        <Field label={t("location")}><SelectInput value={location} onChange={(e) => setLocation(e.target.value as LocationArea)}>{LOCATIONS.map((l) => <option key={l} value={l}>{locationLabel(locale, l)}</option>)}</SelectInput></Field>
        {kind === "artist" ? (<><Field label={t("genre")}><SelectInput value={genre} onChange={(e) => setGenre(e.target.value)}>{GENRE_OPTIONS.map((g) => <option key={g}>{g}</option>)}</SelectInput></Field><Field label={t("label")}><TextInput value={label} onChange={(e) => setLabel(e.target.value)} placeholder={t("independent")} /></Field>{label.toLowerCase().includes("inner soul") ? <p className="text-sm italic text-accent">{t("isrPending")}</p> : null}</>) : null}
        <Field label={t("bio")}><AreaInput rows={4} value={bio} onChange={(e) => setBio(e.target.value)} /></Field>
        {kind === "artist" ? (<div><p className="text-xs text-muted">{t("trackUploadHint")}</p><TextInput className="mt-2" value={trackTitle} onChange={(e) => setTrackTitle(e.target.value)} placeholder={t("trackTitle")} /><label className="relative mt-2 flex h-11 w-full items-center justify-center overflow-hidden rounded-md bg-elevated px-3 text-sm"><span className="pointer-events-none truncate">{fileName ?? t("chooseMp3")}</span><input ref={fileRef} type="file" accept={AUDIO_PICK_ACCEPT} className="absolute inset-0 cursor-pointer opacity-0" onChange={onFile} /></label></div>) : null}
...
function PlainMe() {
  ...
  return (
    <div className="cue-enter pb-12">
      ...
      <LanguageToggle />
      <div className="px-5 pt-4"><Button variant="ghost" className="w-full" onClick={logout}>{t("logOut")}</Button></div>
    </div>
  );
}
