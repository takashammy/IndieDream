...
/** Artists — including Desk staff — may only upload onto their own page. */
export function canUploadToArtist(
  ...
  if (mine !== target) return { ok: false, error: "Songs can only be added to your own page.", status: 403 };
  return { ok: true };
}
